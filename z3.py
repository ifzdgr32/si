...
def eval2(bd,mov,me):
	...
	ret = 0.0
	for i in range(8):
		if b2.pieces[me][i][0] != -1:
			ret += i*100+15
			if me == 0:
				ret += b2.pieces[me][i][1]*i
			else:
				ret += (8-b2.pieces[me][i][1])*i
		if b2.pieces[1-me][i][0] != -1:
			ret -= i*100+15
			if me == 0:
				ret -= b2.pieces[1-me][i][1]*i
			else:
				ret -= (8-b2.pieces[1-me][i][1])*i
	return ret
...
def minmax(bd,mov,me,depth):
	b2 = cpboard(bd)
	res = b2.apply_move(mov[0],mov[1],mov[2])
	if res[0] == True and res[1] == me:
		return maxcof
	if res[0] == True and res[1] != me:
		return mincof
	mv = b2.get_moves()
	if depth <= 0:
		...
		for i in mv:
			ttt = eval2(b2,i,me)
			if me == b2.movpl and limitcof < ttt:
				limitcof = ttt
			if me != b2.movpl and limitcof > ttt:
				limitcof = ttt
		return limitcof
...
def agent2(bd,mv):
	...
	limitcof = mincof
	for i in mv:
		ttt = minmax(bd,i,bd.movpl,startd-1)
		if limitcof < ttt:
			limitcof = ttt
			maksmov = i
	return maksmov
